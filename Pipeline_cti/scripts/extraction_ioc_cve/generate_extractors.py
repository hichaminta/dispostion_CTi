...
sources = [
    ...
    ('OpenPhish', 'openphish_data.json', 'openphish_extractor.py'),
    ('Otx alienvault', 'otx_pulses.json', 'alienvault_extractor.py'),
    ('PhishTank', 'phishtank_data.json', 'phishtank_extractor.py'),
    ('Spamhaus', 'spamhaus_drop.json', 'spamhaus_extractor.py'),
    ('ThreatFox', 'threatfox_data.json', 'threatfox_extractor.py'),
    ('VirusTotal', 'virustotal_data.json', 'virustotal_extractor.py'),
    ('feodotracker', 'feodotracker_data.json', 'feodotracker_extractor.py'),
    ('pulsedive', 'pulsedive_data.json', 'pulsedive_extractor.py'),
    ('url', 'urlhaus_data.json', 'urlhaus_extractor.py')
]
...
OUTPUT_DIR = os.path.join(BASE_DIR, "Pipeline_cti/global_output/output_cve_ioc")
...
def run_extraction():
    ...
    output_path = os.path.join(OUTPUT_DIR, "{source_filename_no_ext}_extracted.json")
    all_results = []
...
# Now the script is in extraction_ioc_cve, so relative to current file
current_dir = os.path.dirname(os.path.abspath(__file__))

for source_dir, json_file, extractor_filename in sources:
    source_name = source_dir
    if source_dir == 'url': source_name = 'URLHaus'
    if source_dir == 'Otx alienvault': source_name = 'OTX AlienVault'
    
    source_filename_no_ext = extractor_filename.replace('_extractor.py', '')
    
    path = os.path.join(current_dir, extractor_filename)
    with open(path, 'w', encoding='utf-8') as f:
        content = template.format(
            source_name=source_name, 
            source_dir=source_dir, 
            json_file=json_file,
            source_filename_no_ext=source_filename_no_ext
        )
        f.write(content)

print("Generated/Updated 13 source-specific extractor scripts.")
