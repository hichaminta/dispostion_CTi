...
OUTPUT_JSON = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "..", "global_output", "sources", "ThreatFox", "collection", "threatfox_data.json"))
...
def load_tracking():
    ...
    tracker = SourceTracker(source_name)
...
def sync_threatfox(raw_list, existing_data, existing_ids, tracking, new_records_total, mode="AFTER"):
    ...
    for i, item in enumerate(raw_list, 1):
        ...
        if ioc_id in existing_ids:
            ...
            old_item = existing_ids[ioc_id]
            old_item.update(item) # Met à jour tous les champs
...
def main():
    ...
    extractor_script = os.path.join(extraction_dir, "threatfox_extractor.py")
    if os.path.exists(extractor_script):
        logging.info(">>> AUTOMATION : Lancement de l'extraction (threatfox_extractor.py)...")
