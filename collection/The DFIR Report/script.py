...
OUTPUT_JSON = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "..", "global_output", "sources", "The DFIR Report", "collection", "dfir_report_data.json"))
...
def load_tracking() -> dict:
    if os.path.exists(TRACKING):
        try:
            ...
            logging.error(f"Erreur lecture tracking: {e}")
...
def save_tracking(data: dict):
    tmp_file = TRACKING + ".tmp"
    try:
        with open(tmp_file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
        os.replace(tmp_file, TRACKING)
    except Exception as e:
        logging.error(f"Erreur sauvegarde tracking : {e}")
