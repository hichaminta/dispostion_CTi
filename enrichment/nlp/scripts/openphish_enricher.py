...
logger = logging.getLogger("Enrichment.OpenPhish")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "openphish_extracted.json")
    ...
    logger.info(f"Checking for new items in: openphish_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for OpenPhish outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for OpenPhish...")
    ...
    out_file_name = "openphish_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "openphish"], capture_output=False)
