...
logger = logging.getLogger("Enrichment.AbuseIPDB")
...
TRACKING_FILE = os.path.join(TRACKING_DIR, "abuseipdb_tracking.json")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "abuseipdb_extracted.json")
    ...
    logger.info(f"Checking for new items in: abuseipdb_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for AbuseIPDB outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for AbuseIPDB...")
    ...
    out_file_name = "abuseipdb_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "abuseipdb"], capture_output=False)
