...
logger = logging.getLogger("Enrichment.Spamhaus")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "spamhaus_extracted.json")
    ...
    logger.info(f"Checking for new items in: spamhaus_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for Spamhaus outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for Spamhaus...")
    ...
    out_file_name = "spamhaus_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "spamhaus"], capture_output=False)
