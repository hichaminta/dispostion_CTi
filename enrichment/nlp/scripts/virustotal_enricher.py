...
logger = logging.getLogger("Enrichment.VirusTotal")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "virustotal_extracted.json")
    ...
    logger.info(f"Checking for new items in: virustotal_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for VirusTotal outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for VirusTotal...")
    ...
    out_file_name = "virustotal_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "virustotal"], capture_output=False)
