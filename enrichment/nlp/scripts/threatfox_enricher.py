...
logger = logging.getLogger("Enrichment.ThreatFox")
...
TRACKING_FILE = os.path.join(TRACKING_DIR, "threatfox_tracking.json")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "threatfox_extracted.json")
    ...
    logger.info(f"Checking for new items in: threatfox_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for ThreatFox outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for ThreatFox...")
    ...
    out_file_name = "threatfox_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "threatfox"], capture_output=False)
