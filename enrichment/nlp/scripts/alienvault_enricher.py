...
logger = logging.getLogger("Enrichment.Otx_alienvault")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "alienvault_extracted.json")
    ...
    logger.info(f"Checking for new items in: alienvault_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for Otx_alienvault outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for Otx_alienvault...")
    ...
    out_file_name = "alienvault_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "alienvault"], capture_output=False)
