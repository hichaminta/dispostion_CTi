...
def process_source():
    ...
    logger.info(f"Checking for new items in: urlhaus_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for url outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for url...")
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "urlhaus"], capture_output=False)
