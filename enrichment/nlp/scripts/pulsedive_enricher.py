...
logger = logging.getLogger("Enrichment.pulsedive")
...
TRACKING_FILE = os.path.join(TRACKING_DIR, "pulsedive_tracking.json")
...
def process_source():
    ...
    file_path = os.path.join(EXTRACTED_DIR, "pulsedive_extracted.json")
    ...
    logger.info(f"Checking for new items in: pulsedive_extracted.json")
    ...
    if not new_items:
        logger.info(f"No new items for pulsedive outside tracked range.")
        return

    logger.info(f"Processing {len(new_items)} new items for pulsedive...")
    ...
    out_file_name = "pulsedive_extracted.json".replace('_extracted.json', '_enriched.json')
    ...
    if os.path.exists(geo_script):
        subprocess.run([sys.executable, geo_script, "--source", "pulsedive"], capture_output=False)
