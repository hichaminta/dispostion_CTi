...
# Ensure we can import the nlp module
...
try:
    ...
    logger.error(f"Failed to import NLPEnricher: {e}")
    sys.exit(1)

def generate_latex():
    logger.info("Generating advanced technical report with code snippets...")
    
    # LaTeX Template with Listings for Code
...
"""
    ...
    # Write .tex file with the NEW NAME
    tex_path = os.path.join(os.path.dirname(__file__), "rapport_technique_nlp.tex")
    ...
    try:
        subprocess.run(["pdflatex", "-interaction=nonstopmode", "rapport_technique_nlp.tex"], cwd=os.path.dirname(__file__), check=True)
        subprocess.run(["pdflatex", "-interaction=nonstopmode", "rapport_technique_nlp.tex"], cwd=os.path.dirname(__file__), check=True)
        logger.info("PDF generated: rapport_technique_nlp.pdf")
